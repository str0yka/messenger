...
export const OutcomingMessage: React.FC<OutcomingMessageProps> = ({ message }) => {
  const messageDate = new Date(message.createdAt);
  const { hours, minutes } = formatTime(messageDate);

  return (
    <div className="w-fit max-w-[66%]  self-end rounded-l-2xl rounded-r-lg bg-primary-500 px-2 py-1 text-white">
      {message.message}
